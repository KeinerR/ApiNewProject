...
function vaciarInputPedidos() {
    ...
    // Reiniciar el ícono del botón de búsqueda
    $('#btnNavbarSearch i').removeClass().addClass('fas fa-search').css('color', 'white');

    // Restaurar el contador y clases adicionales si es necesario
    $('.contador').text('#');
    $('.contadorDos').addClass('noIs');

    // Mostrar todas las filas de la tabla pedidos
    $('.pedidosPaginado').show();

    // Ocultar todas las filas de la tabla Pedidos
    $('.Pedidos').hide();

    // Ocultar el contenedor de paginación si es necesario
    $('#paginationContainer').removeClass('noBe');
}

function searchPedidos() {
    var input = $('#buscarPedidos').val().trim().toLowerCase();

    $('.pedidosPaginado').each(function () {
        var pedidoId = $(this).find('td:nth-child(2)').text().trim().toLowerCase();
        var tipoServicio = $(this).find('td:nth-child(4)').text().trim().toLowerCase();

        if (pedidoId.includes(input) || tipoServicio.includes(input)) {
            $(this).show();
        } else {
            $(this).hide();
        }
    ...
    // No es necesario manipular las filas de la clase 'Pedidos' en esta función, ya que parece que las operaciones están relacionadas con 'pedidosPaginado' solamente.
}
